...
import com.calcite.visitor.ReplaceWhereSqlBasicVisitor;
...
public class SqlValidatorImpl implements SqlValidatorWithHints {
  ...
  protected void validateWhereClause(SqlSelect select) {
    ...
    replaceWhereSqlIdentifier(select, where);
  ...
  private static void replaceWhereSqlIdentifier(SqlSelect select, SqlNode where) {
    Map<String,SqlNode> name2SqlNodeMap =  new HashMap<>();
    SqlNodeList sqlNodes = select.getSelectList();
    List<SqlNode> sqlNodeList = sqlNodes.getList();
    for(SqlNode sqlNode : sqlNodeList){
      if(sqlNode instanceof  SqlBasicCall){
        if(((SqlBasicCall) sqlNode).getKind() == SqlKind.AS){
          SqlNode sqlNode1 =  ((SqlBasicCall) sqlNode).getOperandList().get(0);
          SqlNode sqlNode2 =  ((SqlBasicCall) sqlNode).getOperandList().get(1);
          // slecet 1 as 1
          if(sqlNode1.equalsDeep(sqlNode2,Litmus.IGNORE)){
            continue;
          }
          if(sqlNode2 instanceof  SqlIdentifier){
            name2SqlNodeMap.put(sqlNode2.toString(),sqlNode1);
          }
        }
      }
    }

    where.accept(new ReplaceWhereSqlBasicVisitor(name2SqlNodeMap));
  }
